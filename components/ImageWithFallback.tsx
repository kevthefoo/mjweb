"use client";

import { useState, memo } from "react";
import Image from "next/image";
import image_not_found from "@/assets/image-not-found.webp";

interface ImageWithFallbackProps {
  src: string;
  alt: string;
  height: number;
  width: number;
  priority: boolean;
  className: string;
  index: number;
}

const ImageWithFallback: React.FC<ImageWithFallbackProps> = memo((props) => {
  const { src, alt, height, width, priority, className, index } = props;
  const [imgSrc, setImgSrc] = useState(src);

  const placeholderDataURL =
    "data:image/svg+xml;base64,";

  console.log(
    `Rendering ImageWithFallback for ${alt} with index: ${index}`,
  );

  return (
    <Image
      alt={alt}
      src={imgSrc}
      height={height}
      width={width}
      priority={priority}
      className={className}
      placeholder={placeholderDataURL}
      onError={() => {
        setImgSrc(image_not_found.src);
      }}
    />
  );
});

ImageWithFallback.displayName = "ImageWithFallback";

export default ImageWithFallback;
